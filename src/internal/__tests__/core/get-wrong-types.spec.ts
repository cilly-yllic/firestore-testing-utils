...
describe(__filename, () => {
  it(`simple string`, async () => {
    ...
    expect(
      JSON.stringify(getRecursiveWrongTypes<Data, KeyTypeConst>([DOCUMENT_TYPE], ALL_FIELD_TYPES, INCLUSIONS))
  ...
  it(`complex`, async () => {
    ...
    expect(
      JSON.stringify(getRecursiveWrongTypes<Data, KeyTypeConst>(DOCUMENT_TYPES, ALL_FIELD_TYPES, INCLUSIONS))
    ).toBe(JSON.stringify(LIST))
  })
})
